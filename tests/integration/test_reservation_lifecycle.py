...
pytestmark = pytest.mark.usefixtures("control_service")
...
def _reserve(actor, amount, key, case="c1", ceiling="1000.00"):
    return httpx.post(
...
def test_a_released_hold_frees_the_budget_it_occupied(clean):
    """The core of the leak. Without release, B is refused for an action that
    never happened."""
    first = _reserve("A", "600.00", "k1")
    assert first.status_code == 201

    released = httpx.post(
        f"{CONTROL_URL}/reservations/{first.json()['id']}/release",
        headers=_headers("A"), timeout=30.0,
    )
    assert released.status_code == 200
    assert released.json()["state"] == "RELEASED"

    assert _reserve("B", "600.00", "k2").status_code == 201


def test_an_unreleased_hold_still_blocks(clean):
    """The control must actually be doing something, or the test above proves
    nothing."""
    assert _reserve("A", "600.00", "k1").status_code == 201
    assert _reserve("B", "600.00", "k2").status_code == 409


def test_a_committed_hold_is_not_released(clean):
    """Committed means the money moved. Releasing it would free budget that is
    genuinely spent."""
    first = _reserve("A", "600.00", "k1")
    httpx.post(
        f"{CONTROL_URL}/reservations/{first.json()['id']}/commit",
        headers=_headers("A"), timeout=30.0,
    )
...
def test_releasing_twice_is_idempotent(clean):
    ...
    for _ in range(2):
        r = httpx.post(f"{CONTROL_URL}/reservations/{rid}/release",
                       headers=_headers("A"), timeout=30.0)
        assert r.status_code == 200

    assert _reserve("B", "1000.00", "k2").status_code == 201


def test_releasing_an_unknown_reservation_is_a_404(clean):
    r = httpx.post(f"{CONTROL_URL}/reservations/999999/release",
                   headers=_headers("A"), timeout=30.0)
    assert r.status_code == 404


def test_committing_a_released_hold_is_refused(clean):
    """A released hold has no budget behind it. Committing it would create
    authority from nothing."""
    first = _reserve("A", "600.00", "k1")
    rid = first.json()["id"]
    httpx.post(f"{CONTROL_URL}/reservations/{rid}/release",
               headers=_headers("A"), timeout=30.0)

    r = httpx.post(f"{CONTROL_URL}/reservations/{rid}/commit",
                   headers=_headers("A"), timeout=30.0)
